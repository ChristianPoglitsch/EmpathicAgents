...
def _create_prompt_input(init_scratch:PersonaScratch, 
    ...
    if prev_convo_insert == "\n": 
      prev_convo_insert = ""
    if init_a_mem.seq_chat: 
      if int((init_scratch.curr_time - init_a_mem.seq_chat[-1].created).total_seconds()/60) > 480: 
        prev_convo_insert = ""
    ...
    prompt_input = [init_iss, init_scratch.name, retrieved_str, prev_convo_insert,
      curr_location, curr_context, init_scratch.name, target_scratch.name,
      convo_str, init_scratch.name, target_scratch.name,
      init_scratch.name, init_scratch.name,
      init_scratch.name
      ]
    return prompt_input
...
def run_prompt_generative_iterative_personas_chat(init_scratch:PersonaScratch, 
                                                  ...
                                                  model:LLM_API, 
                                                  verbose=False):
    prompt_template = BASE_DIR + "/LLM_Character/persona/prompt_modules/templates/interative_convo_personas.txt" 
...
if __name__ == "__main__":
    ...
    run_prompt_generative_iterative_personas_chat(person, model, "i will drive to the broeltorens.", "kortrijk" )
