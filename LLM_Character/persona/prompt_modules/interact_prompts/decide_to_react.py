...
def _create_prompt_input(init_scratch: PersonaScratch,
    ...
    if len(init_scratch.planned_path) == 0:
        ...
        if ":" in init_scratch.act_address:
            loc = init_scratch.act_address.split(
                ":")[-1] + " in " + init_scratch.act_address.split(":")[-2]
    ...
    if len(target_scratch.planned_path) == 0:
        ...
        if ":" in target_scratch.act_address:
            loc = target_scratch.act_address.split(
                ":")[-1] + " in " + target_scratch.act_address.split(":")[-2]
        ...
        if ":" in target_scratch.act_address:
            loc = target_scratch.act_address.split(
                ":")[-1] + " in " + target_scratch.act_address.split(":")[-2]
        target_p_desc = f"{target_scratch.name} is on the way to {target_act_desc} at {loc}"
...
def _validate_response(response: str):
    try:
        return response.split(
            "Answer: Option")[-1].strip().lower() in ["3", "2", "1"]
...
def run_prompt_decide_to_react(init_scratch: PersonaScratch,
                               target_scratch: PersonaScratch,
                               retrieved: EventContext,
                               model: LLM_API,
                               verbose=False):
    prompt_template = BASE_DIR + \
        "/LLM_Character/persona/prompt_modules/templates/decide_to_react.txt"
