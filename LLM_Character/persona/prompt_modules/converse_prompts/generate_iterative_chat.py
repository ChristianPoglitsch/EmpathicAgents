...
def _create_prompt_input_1(uscratch: UserScratch,
    ...
    if ca_mem.seq_chat:
        if int((cscratch.curr_time -
               ca_mem.seq_chat[-1].created).total_seconds() / 60) > 480:
...
def run_prompt_iterative_chat(uscratch: UserScratch,
    ...
    prompt_template = BASE_DIR + \
        "/LLM_Character/persona/prompt_modules/templates/is_convo_ending.txt"
    ...
    prompt_template = BASE_DIR + \
        "/LLM_Character/persona/prompt_modules/templates/iterative_convo.txt"
